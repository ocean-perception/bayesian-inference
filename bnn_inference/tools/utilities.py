...
def calc_soft_assignment(samples, centroids, alpha=1.0):
    ...
    :return:
    '''
    assert samples.size()[1] == centroids.size()[
        1], 'num_features should be the same for samples and centroids'
    ...
    soft_assignment = torch.zeros((num_samples, num_classes),
                                  dtype=torch.double)
    for idx_i in range(num_samples):
        ...
        for idx_j in range(num_classes):
            ...
            normalize_term = 1.0
            tmp_numerator[0, idx_j] = \
                (1.0 + torch.sum(((samples[idx_i] - centroids[
                    idx_j]) / normalize_term) ** 2.0) / alpha) \
                ** power_value
...
def calc_kld(soft_assignment, aux_tar_dist):
    assert soft_assignment.size() == aux_tar_dist.size(), 'shape of soft_assignment and aux_tar_dist must be the same'

    # p: auxiliary target distribution
    # q: softmax assignment
    kld = torch.sum(aux_tar_dist * torch.log(aux_tar_dist
                                             / soft_assignment))
...
def calc_d_loss_d_mu(samples, centroids, alpha=1.0):
    ...
    d_loss_d_mu = torch.zeros((num_classes, num_features),
                              dtype=torch.double).cuda()
    for idx_k in range(num_classes):
        tmp_sigma = torch.zeros((1, num_features), dtype=torch.double).cuda()
        for idx_sample in range(num_samples):
...
def calc_t_dstr_from_samples(samples, dstn_max_value=np.inf):
    ...
    xx, yy = np.meshgrid(np.arange(num_samples), np.arange(num_samples))
    numerater_vector = (1.0 + torch.sum((samples[xx] - samples[yy]) ** 2,
                                        dim=2)) ** coef_power
...
def calc_dstn_mat(samples, metric='euclid', dstn_max_value=np.inf):
    ...
    dist = torch.clamp(samples_norm + samples_t_norm
                       - 2.0 * torch.mm(samples, samples_t), 0.0,
                       np.inf)  # clamp is necessary before sqrt for managing very small negative value
...
def calc_kld_t_dstr(p, q, normalize=True):
    ...
    kld = torch.sum(p.view(-1)[idx_triu] * torch.log(
        p.view(-1)[idx_triu] / q.view(-1)[idx_triu]))
