...
def get_treemap_grid(array_num_samples, list_label=None, aspect_ratio=1.33,
                     sort=True):
    '''
    ...
    array_num_samples_norm = squarify.normalize_sizes(array_num_samples, w,
                                                      h)  # Normalize is necessary
    ...
    idxs_edge = -np.ones((len(array_num_samples), 2, 2),
                         dtype=np.int)  # [h_min, w_min], [h_max, w_max]
...
def patch_allocation_pca(w, h, samples):
    ...
    for i_principal in range(
            min(math.ceil(num_samples / secondary), principal)):
...
def get_nine_samples_pil_image(config, base_path, data_frame, samples_latents, patch_size=227,
                               colour=None,
                               num_samples_per_side=3, remove_edge=True,
                               return_image=True):
    '''
    ...
    if return_image:
        # load PIL
        ret_np = np.zeros((patch_size * num_samples_per_side,
                           patch_size * num_samples_per_side, 3),
                          dtype=np.float)
    else:
        ret_array_idx = -np.ones((num_samples_per_side, num_samples_per_side),
                                 dtype=np.uint)
    ...
    for i_h in range(num_samples_per_side):
        for i_w in range(num_samples_per_side):
            ...
            if return_image:
                ...
                ret_np[i_h * patch_size:(i_h + 1) * patch_size,
                i_w * patch_size:(i_w + 1) * patch_size, :] = tmp_img
            else:
...
def get_clustering_tile_pil_image(config, base_path, data_frame, samples_latents,
                                  patch_size_org=227,
                                  max_num_patches=10000, resize_rate=0.1,
                                  list_colour=None, draw_label=True,
    ...
    if len(data_frame) > max_num_patches:
        labels = pd.Series.value_counts(data_frame[result_label],
                                        sort=False).index
        ...
        for i_label in range(len(labels)):
            idx_tmp_label_all = data_frame[
                data_frame[result_label] == labels[i_label]].index
            idx_tmp_label_effective = random.sample(list(idx_tmp_label_all),
                                                    k=int(max_num_patches *
                                                          value_ratio_org[
                                                              i_label]))
            list_effective_idxs.extend(idx_tmp_label_effective)

        data_frame = data_frame.iloc[list_effective_idxs].reset_index(
            drop=True)
    ...
    result = np.ones((patch_size_resized * treemap_grid.shape[0],
                      patch_size_resized * treemap_grid.shape[1], 3),
                     dtype='float16') * 0.3  # default colour is grey
    tf_crop_resize = \
    ...
    for i_label in range(len(labels)):
        ...
        tmp_idx = (data_frame.index[
            data_frame[result_label] == tmp_label]).to_list()
        tmp_img_filepaths =  data_frame.iloc[tmp_idx][
            header.relative_path].reset_index(drop=True)
        ...
        for i_width in range(tmp_w):
            for i_height in range(tmp_h):
                ...
                if sample_idx < 0:
                    tmp_image = \
                        0.3 * np.ones(
                            (patch_size_resized, patch_size_resized, 3),
                            dtype=np.float)  # default colour is grey
                else:
                    tmp_image = tf_crop_resize(
                        Image.open(Path(base_path) / tmp_img_filepaths[sample_idx]))
                    tmp_image = np.array(tmp_image) / 255.0

                tmp_image = add_frame_to_image(tmp_image,
                                               list_colour[tmp_label])
                result[
                (tmp_lu_idx[0] + i_height) * patch_size_resized:(tmp_lu_idx[
                                                                     0] + i_height + 1) * patch_size_resized,
                (tmp_lu_idx[1] + i_width) * patch_size_resized:(tmp_lu_idx[
                                                                    1] + i_width + 1) * patch_size_resized,
                :] = tmp_image
    ...
    for i_label in range(len(labels)):
        ...
        draw.rectangle(
            ...
            fill=None, width=frame_width, outline=tmp_colour_255)
        # draw label
        if draw_label:
            ...
            draw.text((vertex_lu_x + frame_width, vertex_lu_y + frame_width),
                      text_label, fill=tmp_colour_255,
                      font=font)
